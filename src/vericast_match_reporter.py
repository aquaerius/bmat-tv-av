...
class VericastMatchReporter:
    ...
    def __init__(self, args):
        ...
        try:
            ...
            start_time_naive = dateutil.parser.parse(args.start_date+'T'+args.start_time)
            end_time_naive = dateutil.parser.parse(args.end_date+'T'+args.end_time)
            dfeng = pd.DataFrame({'naive_datetime':[start_time_naive,end_time_naive]}, index=['filter_start','filter_end'])
            dfeng['local_datetime'] = pd.DatetimeIndex(dfeng['naive_datetime']).tz_localize(tz =args.time_zone)
            ...
            self.report_name =args.destination_dir+'/'+self.report_name + '.{}'.format(self.report_file_extension)
        except:
            raise
...
def main():
    ...
    try:
        if os.path.isfile(args.filename):
            ...
            args.destination_dir = os.getcwd()
            # Change working directory
            os.chdir(args.filename[0:-len(args.filename.split('/')[-1])])
            args.filename = args.filename.split('/')[-1]
              # Create reporter from file
            reporter = VericastMatchReporter(args)
